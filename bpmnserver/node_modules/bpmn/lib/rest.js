...
/**
 * @api {put} /:processName/:id/:taskName Send data to done a pending task.
 *
 * @apiName DoneTask
 * @apiGroup BPMN
 *
 ...
 * @apiParam {String} processName Name of the process as used in the BPMN model
 * @apiParam {String} id Unique id of the process instance
 * @apiParam {String} taskName Message or taskName as used in the BPMN model for task 
 *
 */
var taskDoneRoute = '/:processName/:id/:taskName';
...
exports.createServer = function(manager, options, restifyOptions) {
    ...
    var settings = options || {};
		console.log("rest is creating Server");
    ...
    server.put(taskDoneRoute,
        transactionLog.transactionLoggerMiddleware({
            name: function(req){
                return 'PUT ' + req.params.taskName + ' task to be done ' + req.params.processName + ' process';
            }
        }),
        function(req, res, next) {
            doneTask(manager, logger, req, res, next);
        }
    );
...
function createProcess(manager, settings, logger, req, res, next) {
		console.log("Server is createProcess for request");
...
function getProcess(manager, req, res, next) {
		console.log("Server is looking for process");
...
//var taskDoneRoute = '/:processName/:id/:taskName';
function doneTask(manager, logger, req, res, next){

    var processId = getParameter(req, "id");
    var processName = getParameter(req, "processName");
    var taskName = getParameter(req, "taskName");
    ...
    manager.get(processId, function(err, bpmnProcess){
        ...
        logger.setProcess(bpmnProcess);
        try {
						bpmnProcess.taskDone(taskName, req.body);
            // 201: Resource (=message) created
            res.send(201, getProcessResponse(bpmnProcess));
        } catch (e) {
            sendError(e, next);
...
function triggerEvent(bpmnProcess, logger, eventName, data) {
		console.log("Server is triggerEvent");
    var message = data || {};
    logger.trace("Triggering event '" + eventName + "'" + JSON.stringify(message));
    bpmnProcess.triggerEvent(eventName, data);
}

function taskDone(bpmnProcess, logger, taskName, data) {
		console.log("Server is taskDone");
    var message = data || {};
    logger.trace("Triggering event '" + eventName + "'" + JSON.stringify(message));
    bpmnProcess.taskDone(taskName, data);
}
